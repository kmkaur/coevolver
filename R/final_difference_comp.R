#Everything Needed for Non-Matching Competition
...
##########PART TWO - DIFFERENCE COMPETITION SPECIFIC FUNCTION##########
coev_div_single_gen <- function(meta_i, meta_j, pars){
  ...
  for(i in 1:pars$N){
    fitness_difference_cost <- fitness_f_diff_cost(pars$zeta_i[i], pars$alpha_i[i], fit_diff[[i]])
    fit_diff_cost_i[[i]] <- fitness_difference_cost
  }
  
  fit_diff_cost_j <- list()
  for(i in 1:pars$N){
    fitness_difference_cost <- fitness_f_diff_cost(pars$zeta_j[i], pars$alpha_j[i], fit_diff[[i]])
  ...
  ptm <- proc.time()
  surv_diff_cost_i <- list()
  for(i in 1:pars$N){
    survivors <- get_survivors(post_sel_i[i], fit_diff_cost_i[[i]])
    surv_diff_cost_i[[i]] <- survivors
  }
  
  surv_diff_cost_j <- list()
  for(i in 1:pars$N){
    survivors <- get_survivors(post_sel_j[i], fit_diff_cost_j[[i]])
    surv_diff_cost_j[[i]] <- survivors
  }
  proc.time()-ptm
  ...
  post_bio_i <- list()
  for(i in 1:pars$N){
    post_bio <- c(unlist(surv_diff_cost_i[[i]]), partners[[i]]$rem_i)
    post_bio_i[[i]] <- post_bio[!is.na(post_bio)]
  }
  
  post_bio_j <- list()
  for(i in 1:pars$N){
    post_bio <- c(unlist(surv_diff_cost_j[[i]]), partners[[i]]$rem_j)
...
coev_div_wrapper <- function(iter, all_pars=NULL, n.gen = 1000, 
  ...
  saveRDS(out, paste0("out_n_c/competition_difference_sim_", iter, ".rds"))
}
...
solo_sim_nc <- coev_div_2(all_pars = NULL, n.gen = 1000, burnin = TRUE, burnin.gen = 200, print = FALSE)
write.csv(solo_sim_nc, file = "solo_sim_nc.csv")
