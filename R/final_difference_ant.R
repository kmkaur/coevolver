#Everything Needed for Non-Matching Antagonism
...
coev_div_single_gen <- function(meta_i, meta_j, pars){
  ...
  fit_diff_ben_j <- list()
  for(i in 1:pars$N){
    fitness_difference_ben <- fitness_f_diff_ben(pars$zeta_j[i], pars$alpha_j[i], fit_diff[[i]])
  ...
  post_bio_j <- list()
  for(i in 1:pars$N){
    post_bio <- c(unlist(surv_diff_ben_j[[i]]), partners[[i]]$rem_j)
...
coev_div_wrapper <- function(iter, all_pars=NULL, n.gen = 1000, 
  ...
  saveRDS(out, paste0("out_n_a/antagonism_difference_sim_", iter, ".rds"))
}
...
solo_sim_na <- coev_div_2(all_pars = NULL, n.gen = 1000, burnin = TRUE, burnin.gen = 200, print = FALSE)
write.csv(solo_sim_na, file = "solo_sim_na.csv")
