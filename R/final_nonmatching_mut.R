#Everything Needed for Non-Matching Mutualism
...
coev_div_single_gen <- function(meta_i, meta_j, pars){
  ...
  fit_diff_ben_i <- list()
  for(i in 1:pars$N){
    fitness_difference_ben <- fitness_f_diff_ben(pars$zeta_i[i], pars$alpha_i[i], fit_diff[[i]])
  ...
  for(i in 1:pars$N){
    survivors <- get_survivors(post_sel_i[i], fit_diff_ben_i[[i]])
...
coev_div_wrapper <- function(iter, all_pars=NULL, n.gen = 1000, 
  ...
  saveRDS(out, paste0("out_n_m/mutualism_not-matching_sim_", iter, ".rds"))
}
...
solo_sim_nm <- coev_div_2(all_pars = NULL, n.gen = 1000, burnin = TRUE, burnin.gen = 200, print = FALSE)
write.csv(solo_sim_mm, file = "solo_sim_nm.csv")

#run the part 4 1000 times
