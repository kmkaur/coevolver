#Everything Needed for Matching Competition
...
##########PART TWO - MATCHING COMPETITION SPECIFIC FUNCTION##########
coev_div_single_gen <- function(meta_i, meta_j, pars){
  ...
  for(i in 1:pars$N){
    fitness_matching_cost <- fitness_f_match_cost(pars$zeta_i[i], pars$alpha_i[i], fit_diff[[i]])
    fit_match_cost_i[[i]] <- fitness_matching_cost
  }
  
  fit_match_cost_j <- list()
  for(i in 1:pars$N){
    fitness_matching_cost <- fitness_f_match_cost(pars$zeta_j[i], pars$alpha_j[i], fit_diff[[i]])
  ...
  post_bio_i <- list()
  for(i in 1:pars$N){
    post_bio <- c(unlist(surv_match_cost_i[[i]]), partners[[i]]$rem_i)
    post_bio_i[[i]] <- post_bio[!is.na(post_bio)]
  }
  
  post_bio_j <- list()
  for(i in 1:pars$N){
    post_bio <- c(unlist(surv_match_cost_j[[i]]), partners[[i]]$rem_j)
...
coev_div_wrapper <- function(iter, all_pars=NULL, n.gen = 1000, 
  ...
  saveRDS(out, paste0("out_m_c/competition_matching_sim_", iter, ".rds"))
}
